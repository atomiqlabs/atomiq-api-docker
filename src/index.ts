...
function validateAndCoerce(
    ...
    for (const [field, rule] of Object.entries(schema)) {
        ...
        if (rule.required && (value === undefined || value === null || value === "")) {
            return {input, error: `Missing required field: '${field}' — ${rule.description}`};
        }
        ...
        switch (rule.type) {
            case "number": {
                ...
                if (isNaN(num)) return {input, error: `Field '${field}' must be a number`};
                input[field] = num;
                break;
            }
            case "boolean": {
                ...
                else return {input, error: `Field '${field}' must be a boolean`};
                break;
            }
            case "array":
                if (!Array.isArray(value)) return {input, error: `Field '${field}' must be an array`};
                break;
            case "object":
                if (typeof value !== "object" || Array.isArray(value)) return {input, error: `Field '${field}' must be an object`};
                break;
        }
    }
